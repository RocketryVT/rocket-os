...
def receive_message(msg):
    ...
    # Whether pressure release admin has control (True) or not (False)
...
def pressure_ok():
    ...
    rospy.loginfo(
        "vent valve is not longer controled by pressure release admin")
    pub.publish(1)
...
def publish_pressure(event):
    ...
    if active_control:
        rospy.loginfo("Control Pressure Status: " + str(active_control) +
                      " and vibing, Pressure: " + str(current_pressure))
...
if __name__ == "__main__":
    ...
    # Set Subscription
    sub = rospy.Subscriber("/sensors/ox_tank_transducer",
                           SensorReading, receive_message)
