...
def get_command(message):
    ...
    if command == "launch" and countdown_timer is None:
        rospy.logwarn(
            "Beginning launch countdown. Any user input will abort the launch.")
        start_timer()
...
if __name__ == "__main__":
    ...
    # Set Publisher
    pub_commands = rospy.Publisher(
        "/requested_commands", String, queue_size=10)

    # launch involves injection valve, ematch, abort valve
    injection_valve = rospy.Publisher(
        "/hardware/injection_valve/request", UInt8, queue_size=10)
    abort_valve = rospy.Publisher(
        "/hardware/abort_valve/request", UInt8, queue_size=10)
