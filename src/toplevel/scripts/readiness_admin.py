...
def receive_command(string):
    ...
    if string == "print whitelist":
        ...
        rospy.loginfo("Current readiness level is " +
                      str(global_readiness_level))
...
def get_requested_command(message):
    ...
    if matches:
        rospy.logdebug(
            "Command '{}' matches these patterns: {}".format(command, matches))
    ...
    # No matches to command parameter
        ...
        rospy.logwarn(
            "Command doesn't match any patterns in the current whitelist.")
